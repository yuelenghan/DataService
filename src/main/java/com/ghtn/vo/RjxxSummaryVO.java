package com.ghtn.vo;

/**
 * User: Administrator
 * Date: 14-2-17
 * Time: 下午4:31
 */
public class RjxxSummaryVO {

    private String mainDeptId;
    private String deptName;
    private String personNumber;
    private String name;
    private String posName;
    private String needFreq;
    private String rjAll;
    private String yb;
    private String zb;
    private String zhb;
    private String planFreq;
    private String dbrj;
    private String needHour;
    private String rjsj;
    private String pjsj;
    private String yhAll;
    private String swAll;

    public String getMainDeptId() {
        return mainDeptId;
    }

    public void setMainDeptId(String mainDeptId) {
        this.mainDeptId = mainDeptId;
    }

    public String getDeptName() {
        return deptName;
    }

    public void setDeptName(String deptName) {
        this.deptName = deptName;
    }

    public String getPersonNumber() {
        return personNumber;
    }

    public void setPersonNumber(String personNumber) {
        this.personNumber = personNumber;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getPosName() {
        return posName;
    }

    public void setPosName(String posName) {
        this.posName = posName;
    }

    public String getNeedFreq() {
        return needFreq;
    }

    public void setNeedFreq(String needFreq) {
        this.needFreq = needFreq;
    }

    public String getRjAll() {
        return rjAll;
    }

    public void setRjAll(String rjAll) {
        this.rjAll = rjAll;
    }

    public String getYb() {
        return yb;
    }

    public void setYb(String yb) {
        this.yb = yb;
    }

    public String getZb() {
        return zb;
    }

    public void setZb(String zb) {
        this.zb = zb;
    }

    public String getZhb() {
        return zhb;
    }

    public void setZhb(String zhb) {
        this.zhb = zhb;
    }

    public String getPlanFreq() {
        return planFreq;
    }

    public void setPlanFreq(String planFreq) {
        this.planFreq = planFreq;
    }

    public String getDbrj() {
        return dbrj;
    }

    public void setDbrj(String dbrj) {
        this.dbrj = dbrj;
    }

    public String getNeedHour() {
        return needHour;
    }

    public void setNeedHour(String needHour) {
        this.needHour = needHour;
    }

    public String getRjsj() {
        return rjsj;
    }

    public void setRjsj(String rjsj) {
        this.rjsj = rjsj;
    }

    public String getPjsj() {
        return pjsj;
    }

    public void setPjsj(String pjsj) {
        this.pjsj = pjsj;
    }

    public String getYhAll() {
        return yhAll;
    }

    public void setYhAll(String yhAll) {
        this.yhAll = yhAll;
    }

    public String getSwAll() {
        return swAll;
    }

    public void setSwAll(String swAll) {
        this.swAll = swAll;
    }
}
